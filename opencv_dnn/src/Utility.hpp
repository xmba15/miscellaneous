...
generateColorCharts(const uint16_t numClasses = 1000, const uint16_t seed = 255)
{
    std::srand(seed);
    std::vector<std::array<int, 3>> colors;
    colors.reserve(numClasses);
    for (uint16_t i = 0; i < numClasses; ++i) {
        colors.emplace_back(std::array<int, 3>{
...
toCvScalarColors(const std::vector<std::array<int, 3>> &colors)
{
    std::vector<cv::Scalar> result;
    result.reserve(colors.size());
    std::transform(
        std::begin(colors), std::end(colors), std::back_inserter(result),
