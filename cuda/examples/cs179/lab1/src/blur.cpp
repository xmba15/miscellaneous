...
int large_gauss_test(int argc, char **argv)
{
    ...
    // Read audio
    float *all_channel_input = new float[in_file_info.frames * in_file_info.channels];
    amt_read = sf_read_float(in_file, all_channel_input, in_file_info.frames * in_file_info.channels);
    assert(amt_read == in_file_info.frames * in_file_info.channels);

    // Prepare output storage
    float *all_channel_output = new float[in_file_info.frames * in_file_info.channels];
    ...
    for (int ch = 0; ch < n_channels; ch++) {
        ...
        const unsigned int blocks = std::min(max_blocks, (unsigned int)ceil(n_frames / (float)local_size));

        // Call our exposed entry point to our GPU kernel handler
        float gpu_time_milliseconds =
            cuda_call_blur_kernel(blocks, local_size, input_data, blur_v, output_data, n_frames, GAUSSIAN_SIZE);
        ...
        for (int i = 0; i < n_frames; i++) {
            if (fabs(output_data_host[i] - output_data[i]) < 1e-6) {
                ...
                success = false;
                cerr << "Incorrect output at index " << i << ": " << output_data_host[i] << ", " << output_data[i]
                     << endl;
            }
        }
