...
int main(int argc, char *argv[])
{
    ...
    if (!(size_to_run == -1 || size_to_run == 512 || size_to_run == 1024 || size_to_run == 2048 ||
          size_to_run == 4096)) {
        fprintf(stderr, "Program only designed to run sizes 512, 1024, 2048, 4096\n");
    }

    assert(kernel == "all" || kernel == "cpu" || kernel == "gpu_memcpy" || kernel == "naive" || kernel == "shmem" ||
           kernel == "optimal");
...
#define STOP_RECORD_TIMER(name)                                                                                        \
    {                                                                                                                  \
        ...
        // Copy input to GPU
        gpuErrChk(cudaMemcpy(d_input, input, n * n * sizeof(float), cudaMemcpyHostToDevice));
        ...
        if (kernel == "gpu_memcpy" || kernel == "all") {
            START_TIMER();
            gpuErrChk(cudaMemcpy(d_output, d_input, n * n * sizeof(float), cudaMemcpyDeviceToDevice));
        ...
        if (kernel == "naive" || kernel == "all") {
            ...
            gpuErrChk(cudaMemcpy(output, d_output, n * n * sizeof(float), cudaMemcpyDeviceToHost));
            checkTransposed(input, output, n);
